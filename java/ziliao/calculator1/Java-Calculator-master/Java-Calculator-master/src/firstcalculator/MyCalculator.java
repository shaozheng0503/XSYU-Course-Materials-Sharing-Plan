		...
		public void actionPerformed(ActionEvent e) {
			...
			// 处理十进制转换
				...
				try {
					...
					JOptionPane.showMessageDialog(null, "请输入有效的二进制数", "Error!", JOptionPane.ERROR_MESSAGE);
					throw new NumberFormatException("无效的格式");
				} finally {
					start = true;
				}
			}
			...
			// 处理阶乘
				...
				if (Double.parseDouble(jTextField.getText()) < 0) {
					JOptionPane.showMessageDialog(null, "阶乘的值不能为负数", "Error!", JOptionPane.ERROR_MESSAGE);
					jTextField.setText("阶乘的值不能为负数");
					start = true;
					throw new IllegalArgumentException("阶乘必须是非负整数");
				} else {
					int sum;
				...
				if (!start) {
					if (command.equals("+")) {
						...
						if (Double.parseDouble(jTextField.getText()) != 0) {
							...
							JOptionPane.showMessageDialog(null, "除数不能为零", "Error!", JOptionPane.ERROR_MESSAGE);
							command = "=";
							start = true;
