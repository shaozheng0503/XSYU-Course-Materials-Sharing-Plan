...
def main():
    try:
        ...
        sheet_name = 'w4数据'  # 修改为 w2 数据的工作表名
        df = read_data(file_path, sheet_name)

        # 2. 数据预处理
        feature_columns = [
            '深度', '真实垂直深度', '最近5英尺的钻进速率', '自然伽马', '34声波高度',
            '16光电系数', '28光电系数', '34光电系数', '电阻率',
            '井温'
        ]
        target_columns = [
            '电阻率', '中子', '总孔隙度', '密度', '中子',
        ...
        # 3. 划分训练集和测试集
        X_train, X_test, y_train, y_test = train_test_split(X_scaled, y[~y.isna().any(axis=1)], test_size=0.2,
                                                            random_state=42)
