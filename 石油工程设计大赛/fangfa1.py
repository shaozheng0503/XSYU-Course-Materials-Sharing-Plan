...
def preprocess_data(df, feature_columns, target_columns):
    ...
    # 处理井径列中的异常数据
    X.loc[:, '井径'] = X['井径'].astype(str).str.split().str[0]
    ...
    for col in X.columns:
        ...
        X[col] = np.where((X[col] < lower_bound) | (X[col] > upper_bound) | (z_scores > 3), np.nan, X[col])
        X = X.fillna(X.median())
...
def main():
    try:
        ...
        feature_columns = [
                    ...
                    '光电吸收界面指数', '深侧向电阻率', '微球型聚焦电阻率', '浅侧向电阻率',
        ...
        target_columns = [
                ...
                models = {
                    ...
                    "MLP": {
                        "model": MLPRegressor(random_state=42, max_iter=3000, learning_rate_init=0.001),
                        "param_grid": {
